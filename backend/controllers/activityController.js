...
export const getActivity = catchAsync(async (req, res, next) => {
  ...
  const userPosts = await Post.find({
    ...
    .select("_id caption image user likes comments createdAt");

  const postActivities = userPosts.map((post) => ({
    type: "post",
    post: post, // ← Just pass the whole post object
  ...
  const userComments = await Comment.find({
    ...
    .populate({
      path: "post",
      select: "caption image user likes comments",
      populate: [
        {
          path: "user",
          select: "username profilePicture",
  ...
  const commentActivities = userComments.map((comment) => ({
    ...
    post: comment.post, // Now has fully populated comments
